...
class NeuroCAASLogObject(object):
    ...
    def __init__(self,s3_path,write_localpath):
        ...
        try:
            uriparse = urlparse(self.s3_path,allow_fragments=False)
            self.bucket_name = uriparse.netloc
            self.path = uriparse.path.lstrip("/")
            rawfile = self.load()
            #rawcert = load_cert(bucket_name,path)
            self.rawfile = rawfile
            #self.rawcert = rawcert
            writeobj_dict["loc"] = "s3"
            writeobj_dict["bucket"] = self.bucket_name
            writeobj_dict["key"] = self.path
            ...
            raise AssertionError("Error getting {self.logtype} object") 
...
class NeuroCAASDataStatus(NeuroCAASLogObject):
    ...
    :param container: docker container object that we will be querying for relevant status information. Can be none if reading  
